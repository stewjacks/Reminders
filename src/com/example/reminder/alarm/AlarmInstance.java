...
public final class AlarmInstance implements AlarmInterface.InstancesColumns {
    /**
...
//                        + instance);
    ...
    public AlarmInstance(Calendar calendar, int id) {
        this(calendar);
        mAlarmId = id;
    }
    ...
    public AlarmInstance(Cursor c) {
        ...
        if (!c.isNull(ALARM_ID_INDEX)) {
            mAlarmId = c.getInt(ALARM_ID_INDEX);
        }
    ...
    public Calendar getTimeout(Context context) {
        String timeoutSetting = PreferenceManager.getDefaultSharedPreferences(context)
                .getString(Constants.KEY_AUTO_SILENCE, DEFAULT_ALARM_TIMEOUT_SETTING);
